...
pub fn chunk_directory(dir_path: &Path) -> Vec<Chunk> {
    ...
    let valid_exts = [
        ...
        "h", "hpp", "cs", "php", "swift", "kt", "md", "txt", "json",
...
pub struct VectorStore {
    ...
    /// Path where the index is persisted between restarts
...
impl VectorStore {
    ...
    /// Remove all records belonging to a directory (before re-indexing)
    pub async fn clear_directory(&self, dir: &str) {
        let mut records = self.records.write().await;
        records.retain(|r| !r.chunk.file_path.starts_with(dir));
    }

    /// Embed and store chunks; calls `progress_cb(done, total)` after each chunk
        ...
        F: Fn(usize, usize) + Send + Sync,
    {
        ...
        for chunk in &chunks {
            if let Ok(emb) = self.embed_text(&chunk.content).await {
                let mut records = self.records.write().await;
                records.push(VectorRecord { chunk: chunk.clone(), embedding: emb });
            }
            done += 1;
    ...
    /// Persist current index to disk (called after indexing + on app close)
    pub async fn save(&self) -> Result<(), String> {
        let records = self.records.read().await;
        let persisted = PersistedStore { records: records.clone() };
    ...
    // ─────────────────────────────────────────
    // RETRIEVER — active-file & recent-file boosting
    // ─────────────────────────────────────────

    /// Search for the `k` most relevant chunks.
    /// - Active file (currently open in editor) → 1.5× boost
    /// - Recent files (open tabs)               → 1.2× boost
    pub async fn search(
        &self,
        query: &str,
        k: usize,
        ...
        let mut results: Vec<(Chunk, f32)> = records.par_iter().map(|record| {
